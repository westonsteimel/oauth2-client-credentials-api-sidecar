...
func getOuath2AuthAccessToken() {
	...
	if auth_scope != "" {
		data.Set("scope", auth_scope)
	}

	//pass the values to the request's body
	req, err := http.NewRequest("POST", auth_endpoint_url, strings.NewReader(data.Encode()))
	if err != nil {
		log.Fatal(err)
	}
	req.SetBasicAuth(auth_client_id, auth_client_secret)
